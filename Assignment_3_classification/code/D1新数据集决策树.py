...
# 读取iris.data文件
D1 = pd.read_csv('data/raw/cmc.data', header=None)
# 添加列名
X = D1.iloc[:,:-1].values
y = D1.iloc[:,-1].values
le = LabelEncoder()
...
ax.set_xticklabels(['No-use ','Long-term','Short-term'])
ax.set_yticklabels(['No-use ','Long-term','Short-term'])
# 在每个方格内显示数值
thresh = cm.max() / 2
...
plt.savefig('./img/decisiontree_cmc_confusion_matrix.eps', format='eps', dpi=100, bbox_inches='tight')
plt.show()
plt.close()
...
plt.savefig('./img/decisiontree_cmc_evaluation_metrics.eps', format='eps', dpi=1000, bbox_inches='tight')

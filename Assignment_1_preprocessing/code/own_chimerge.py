    ...
    for index, row in df.iterrows():  # iterating over old dataframe
        if (index == 0):
            ...
            if (df.loc[index - 1]['chi2'] == distinct_values[0]):  # merge
                updated_df.loc[updated_df_index -
                               1]['Iris-setosa'] += df.loc[index]['Iris-setosa']
                updated_df.loc[updated_df_index -
                               1]['Iris-versicolor'] += df.loc[index]['Iris-versicolor']
                updated_df.loc[updated_df_index -
                               1]['Iris-virginica'] += df.loc[index]['Iris-virginica']
            else:
                updated_df.loc[len(updated_df)] = df.loc[index]
                updated_df_index += 1
...
def create_contingency_table(dataframe, feature):
    ...
    col_names = [feature, 'Iris-setosa',
                 'Iris-versicolor', 'Iris-virginica', 'chi2']
    ...
    for i in range(len(distinct_values)):
        ...
        count_dict = temp_df["class"].value_counts().to_dict()
        ...
        new_row = [distinct_values[i], setosa_count,
                   versicolor_count, virginica_count, 0]
        my_contingency.loc[len(my_contingency)] = new_row

    return my_contingency


def my_chimerge(feature, data, max_interval):
    df = data.sort_values(by=[feature], ascending=True).reset_index()
    # generate frequency table
    contingency_table = create_contingency_table(df, feature)
    # calculate initial number of intervals. #initially, all entries are intervals on their own
    ...
    for index, row in contingency_table.iterrows():
        ...
        split_point.append(contingency_table.loc[index][feature])

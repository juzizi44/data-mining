...
D3 = pd.read_csv('data/raw/winequality-red.csv', delimiter=';')
X = D3.iloc[:,:-1].values
y = D3.iloc[:,-1].values
# 将数据集分成训练集和测试集
...
# 初始化神经网络
nn = NeuralNetwork(input_dim=11, hidden_dim=10, output_dim=11)
...
for i in range(1000):
    ...
    # 计算损失函数值
    loss = nn.loss(y_pred_train, np.eye(11)[y_train])

    # 反向传播
    nn.backward(X_train, np.eye(11)[y_train], y_pred_train, a1_train)
...
plt.savefig('./img/net_wine_confusion_matrix.eps', format='eps', dpi=100, bbox_inches='tight')
plt.show()
plt.close()
...
plt.savefig('./img/net_wine_evaluation_metrics.eps', format='eps', dpi=1000, bbox_inches='tight')
plt.show()
plt.close()
